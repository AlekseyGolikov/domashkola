...
def login():
    ...
    if form.validate_on_submit():
        ...
        # print('User {} password {} check_password'.format(user.username, user.password))
        if user is None or not user.check_password(password=form.password.data):
            flash('Вы ввели неправильные логин или пароль!')
    ...
    return render_template(url_for('users.login')+".html", title='Авторизация', form=form, page=page)
